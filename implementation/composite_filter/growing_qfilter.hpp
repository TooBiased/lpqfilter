...
class concurrent_growing_handler
{
  public:
    ...
    class growing_status_data
    {
      ...
      public:
        ...
        {
            return static_cast<bool>(data & growing_mask);
        }
  ...
  public:
    concurrent_growing_handler(size_t index) : atomic_data(index & index_mask)
    {
    }
    ...
    concurrent_growing_handler&
    operator=(const concurrent_growing_handler& other)
    {
        atomic_data = other.atomic_data.load();
        return *this;
    }
    ...
    concurrent_growing_handler&
    operator=(concurrent_growing_handler&&) = default;
    ...
    void end_growing() { atomic_data = atomic_data.load().index() + 1; }
};
...
template <class growing_filter_base> class non_templated_filter_creator
{
  public:
    static void create_filter(growing_filter_base& ref, size_t current_index)
    {
        ...
        ref.filters[current_index].store(new quotient_filter(
            ONE << ref.quotient_bits,
            ref.index_to_reamainder_bits(current_index), ref.hf));
    }
...
template <class growing_filter_base> class templated_filter_creator
{
  public:
    template <size_t rbits>
    using quotient_filter_template =
        typename growing_filter_base::quotient_filter::template instanciated<
            rbits>;
                              ...
                              std::index_sequence<Indices...>)
    {
        ...
        ((current_index == Indices &&
          (ref.filters[Indices] = new quotient_filter_template<
...
class growing_quotient_filter_base
{
  private:
    ...
    using rec_strat =
        typename std::conditional<quotient_filter::is_sequential,
                                  qf::seq_reclamation::seq_reclamation_strategy,
                                  reclamation_strategy>::type;

    using rec_pointer =
        typename rec_strat::template atomic_reclamation<quotient_filter>;
    using rec_handle =
        typename rec_strat::template reclamation_handle<quotient_filter>;
    using rec_manager =
        typename rec_strat::template reclamation_manager<quotient_filter>;


    using filter_creator = typename std::conditional<
        quotient_filter::is_templated, templated_filter_creator<this_type>,
        non_templated_filter_creator<this_type> >::type;
    friend filter_creator;


    using growing_handler_type = concurrent_growing_handler;
    using growing_status_data =
        typename growing_handler_type::growing_status_data;
  ...
  public:
    ...
    growing_quotient_filter_base(
        size_t quotient_bits, size_t remainder_bits, size_t min_remainder_bits,
        const hash_function_type& hf = hash_function_type());
    ...
    growing_quotient_filter_base&
    operator=(const growing_quotient_filter_base&) = delete;
    ...
    inline bool insert_hash(const hashed_type& hashed, rec_handle& handle);
    inline qf::ContainsResult
  ...
  private:
    ...
    rec_handle get_handle_internal() { return filter_manager.get_handle(); }
...
growing_quotient_filter_base<base_filter, rec_strat>::
    growing_quotient_filter_base(size_t min_capacity, size_t remainder_bits,
                                 const hash_function_type& hf)
    : max_index(filters.size() - 1), hf(hf),
...
template <class base_filter, class rec_strat>
growing_quotient_filter_base<base_filter, rec_strat>::
    growing_quotient_filter_base(size_t quotient_bits, size_t remainder_bits,
                                 size_t                    min_remainder_bits,
                                 const hash_function_type& hf)
    : quotient_bits(quotient_bits),
      max_index(remainder_bits_to_index(min_remainder_bits)), hf(hf),
      growing_handler(remainder_bits_to_index(remainder_bits))
{
    auto index = remainder_bits_to_index(remainder_bits);
...
growing_quotient_filter_base<base_filter, rec_strat>::
    ...
    : workers_start(false), quotient_bits(source.quotient_bits),
      max_index(source.max_index),
      cluster_length_limit(source.cluster_length_limit),
      block_size(source.block_size), hf(source.hf),
      growing_handler(std::move(source.growing_handler)), filter_manager()
{
...
template <class base_filter, class rec_strat>
growing_quotient_filter_base<base_filter, rec_strat>&
growing_quotient_filter_base<base_filter, rec_strat>::operator=(
    growing_quotient_filter_base&& source)
{
...
template <class base_filter, class rec_strat>
growing_quotient_filter_base<base_filter,
                             rec_strat>::~growing_quotient_filter_base()
{
...
template <class base_filter, class rec_strat>
void growing_quotient_filter_base<base_filter, rec_strat>::prefetch(
    const hashed_type& hashed, rec_handle& handle)
{
...
template <class base_filter, class rec_strat>
void growing_quotient_filter_base<base_filter, rec_strat>::unsafe_prefetch(
...
template <class base_filter, class rec_strat>
bool growing_quotient_filter_base<base_filter, rec_strat>::insert_hash(
    const hashed_type& hashed, rec_handle& handle)
{
    ...
    if constexpr (is_sequential)
    {
        auto current_status = growing_handler.status();
        auto result =
            filters[current_status.index()].load()->insert_hash(hashed);
...
template <class base_filter, class rec_strat>
qf::ContainsResult
growing_quotient_filter_base<base_filter, rec_strat>::contains_hash(
    const hashed_type& hashed, rec_handle& handle)
{
    if constexpr (is_sequential)
    {
        auto current_status = growing_handler.status();
        // std::cout << "contains start  cindex:" << current_status.index() <<
        // std::flush;
        auto result =
            filters[current_status.index()].load()->contains_hash(hashed);
...
template <class base_filter, class rec_strat>
bool growing_quotient_filter_base<base_filter, rec_strat>::quick_insert_hash(
    const hashed_type& hashed)
{
...
template <class base_filter, class rec_strat>
qf::ContainsResult
growing_quotient_filter_base<base_filter, rec_strat>::unsafe_contains_hash(
    const hashed_type& hashed)
{
    if constexpr (is_sequential)
...
template <class base_filter, class rec_strat>
bool growing_quotient_filter_base<base_filter, rec_strat>::grow(
    growing_handler_type::growing_status_data& current_status,
    rec_handle&                                handle)
{
    if constexpr (is_sequential)
    {
    ...
    else
    {
        ...
        if (current_qf_ptr == nullptr) return true;
        ...
        if (task == growing_handler_type::growing_task::master)
        {
            ...
            block_size =
                std::min(new_capacity + shift_buffer, GROWING_BLOCK_SIZE);
            quotient_bits++;
...
template <class base_filter, class rec_strat>
void growing_quotient_filter_base<base_filter, rec_strat>::create_next_filter(
...
template <class base_filter, class rec_strat>
void growing_quotient_filter_base<base_filter, rec_strat>::create_filter(
    size_t current_index)
{
...
template <class base_filter, class rec_strat>
void growing_quotient_filter_base<
    base_filter, rec_strat>::handle_type::prefetch(const hashed_type& hashed)
{
    return qf.prefetch(hashed, handle);
}

template <class base_filter, class rec_strat>
bool growing_quotient_filter_base<base_filter, rec_strat>::handle_type::insert(
    const key_type& key)
{
...
template <class base_filter, class rec_strat>
bool growing_quotient_filter_base<
    base_filter, rec_strat>::handle_type::insert_hash(const hashed_type& hashed)
{
...
template <class base_filter, class rec_strat>
bool growing_quotient_filter_base<
    base_filter, rec_strat>::handle_type::quick_insert(const key_type& key)
{
    return qf.quick_insert(key);
}

template <class base_filter, class rec_strat>
bool growing_quotient_filter_base<base_filter, rec_strat>::handle_type::
...
template <class base_filter, class rec_strat>
qf::ContainsResult
growing_quotient_filter_base<base_filter, rec_strat>::handle_type::contains(
    const key_type& key)
{
...
template <class base_filter, class rec_strat>
qf::ContainsResult growing_quotient_filter_base<base_filter, rec_strat>::
    handle_type::contains_hash(const hashed_type& hashed)
{
    return qf.contains_hash(hashed, handle);
}

template <class base_filter, class rec_strat>
qf::ContainsResult growing_quotient_filter_base<
    base_filter, rec_strat>::handle_type::unsafe_contains(const key_type& key)
{
...
template <class base_filter, class rec_strat>
qf::ContainsResult growing_quotient_filter_base<base_filter, rec_strat>::
    handle_type::unsafe_contains_hash(const hashed_type& hashed)
{
...
template <class base_filter, class rec_strat>
size_t
growing_quotient_filter_base<base_filter, rec_strat>::handle_type::capacity()
    const
{
    return qf.capacity();
}
...
template <class base_filter>
using growing_quotient_filter =
    growing_quotient_filter_base<base_filter, default_reclamation_strategy>;
