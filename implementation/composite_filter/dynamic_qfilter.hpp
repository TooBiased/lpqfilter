...
namespace qf
{
template <class Dynamic_QF> class dynamic_quotient_filter_handle
{
  private:
    friend Dynamic_QF;
  ...
  public:
    dynamic_quotient_filter_handle(Dynamic_QF& qf, handle_type&& handle,
                                   size_t level)
        : qf(qf), handle(std::move(handle)), level(level){};

    bool insert(const key_type& key) { return qf.insert(key, *this); }

    auto contains(const key_type& key) { return qf.contains(key, *this); }

    size_t capacity() const { return qf.capacity(); }

    size_t memory_usage_bytes() const { return qf.memory_usage_bytes(); }

    size_t unused_memory_bits() const { return qf.unused_memory_bits(); }

    double fill_level() const { return qf.fill_level(); }
};
...
class dynamic_quotient_filter_base
{
  ...
  public:
    ...
    using hashed_value_type =
        typename std::invoke_result<hash_function_type, key_type>::type;
    using handle_type = dynamic_quotient_filter_handle<this_type>;
    friend handle_type;


    dynamic_quotient_filter_base(
        double false_positive_rate = 0.01, size_t min_capacity = 2ull << 8,
    ...
    dynamic_quotient_filter_base&
    operator=(dynamic_quotient_filter_base&& other);
    dynamic_quotient_filter_base&
    operator=(const dynamic_quotient_filter_base&) = delete;
  ...
  private:
    void prefetch(hashed_value_type hashed, size_t lvl,
...
dynamic_quotient_filter_base<QF, qi, pf>::dynamic_quotient_filter_base(
    double false_positive_rate, size_t min_capacity,
    const hash_function_type& hf)
    : hf(hf)
{
    const size_t lvl0_min_r_bits =
        1 + false_positive_rate_to_remainder_bits(false_positive_rate);
    lvl0_initial_r_bits = lvl0_min_r_bits + QUOTIENT_BIT_STEP_SIZE;
    // std::cout << "starting rbits" << lvl0_min_r_bits << std::endl;
    lvl0_initial_q_bits = std::max(MIN_INITIAL_QUOTIENT_BITS,
                                   capacity_to_quotient_bits(min_capacity));

    level_count =
        (BASE_INTEGER_BITS - lvl0_initial_r_bits - lvl0_initial_q_bits) /
        (QUOTIENT_BIT_STEP_SIZE + 1);
    levels = std::make_unique<level_ptr_type[]>(level_count);

    levels[0] = std::make_unique<growing_filter_type>(
        lvl0_initial_q_bits, lvl0_initial_r_bits, lvl0_min_r_bits);
}
...
dynamic_quotient_filter_base<QF, qi, pf>::dynamic_quotient_filter_base(
    ...
    : hf(other.hf), lvl0_initial_q_bits(other.lvl0_initial_q_bits),
      lvl0_initial_r_bits(other.lvl0_initial_r_bits),
      level_count(other.level_count), levels(std::move(other.levels)),
...
dynamic_quotient_filter_base<QF, qi, pf>::operator=(
    dynamic_quotient_filter_base&& other)
{
    hf                  = other.hf;
...
template <class QF, bool qi, bool pf>
void dynamic_quotient_filter_base<QF, qi, pf>::prefetch(
    hashed_value_type hashed, size_t max_lvl, handle_type& dyn_handle) const
{
...
template <class QF, bool qi, bool pf>
bool dynamic_quotient_filter_base<QF, qi, pf>::insert(const key_type& key,
                                                      handle_type& dyn_handle)
{
    ...
    {
        for (size_t current_level = 0; current_level < local_completed_index;
             current_level++)
        {
            if (levels[current_level]->quick_insert_hash(hashed)) return true;
        }
    }

    do {
        update_handle(dyn_handle);
        if (dyn_handle.handle.insert_hash(hashed)) return true;
...
template <class QF, bool qi, bool pf>
bool dynamic_quotient_filter_base<QF, qi, pf>::insert_grow_test(
    const key_type& key)
{
    ...
    if constexpr (quick_insert_enabled)
    {
        for (size_t current_level = 0; current_level < completed_index;
             current_level++)
        {
            if (levels[current_level]->quick_insert(key)) return true;
        }
    }
...
template <class QF, bool qi, bool pf>
void dynamic_quotient_filter_base<QF, qi, pf>::grow()
{
    if constexpr (!is_sequential) return;

    if (!levels[completed_index]->grow()) add_level();
}



template <class QF, bool qi, bool pf>
bool dynamic_quotient_filter_base<QF, qi, pf>::contains(const key_type& key,
                                                        handle_type& dyn_handle)
{
    const auto   hashed                = hf(key);
    const size_t local_completed_index = completed_index;
    ...
    for (size_t current_level = 0; current_level < local_completed_index;
         current_level++)
    {
        const auto contains_result =
            levels[current_level]->unsafe_contains_hash(hashed).result();
...
template <class QF, bool qi, bool pf>
size_t dynamic_quotient_filter_base<QF, qi, pf>::capacity() const
{
...
template <class QF, bool qi, bool pf>
size_t dynamic_quotient_filter_base<QF, qi, pf>::memory_usage_bytes() const
{
    size_t mem_usage = 0;
...
template <class QF, bool qi, bool pf>
size_t dynamic_quotient_filter_base<QF, qi, pf>::unused_memory_bits() const
{
...
template <class QF, bool qi, bool pf>
double dynamic_quotient_filter_base<QF, qi, pf>::fill_level() const
{
...
dynamic_quotient_filter_base<QF, qi, pf>::get_handle()
{
    ...
    return handle_type(*this,
                       create_handle(*levels[local_completed_index].get()),
...
template <class QF, bool qi, bool pf>
void dynamic_quotient_filter_base<QF, qi, pf>::update_handle(
    handle_type& dyn_handle)
{
    const size_t local_completed_index = completed_index;
...
template <class QF, bool qi, bool pf>
bool dynamic_quotient_filter_base<QF, qi, pf>::add_level(size_t next_level)
{
    if (next_level <= completed_index) return true;

    if (next_level >= level_count) return false;

    size_t current_max_level = next_level - 1;

    if (!in_progress_index.compare_exchange_strong(current_max_level,
                                                   next_level))
    {
    ...
    const size_t initial_q_bits =
        lvl0_initial_q_bits + next_level * QUOTIENT_BIT_STEP_SIZE;
    const size_t initial_r_bits = lvl0_initial_r_bits + next_level;
    const size_t min_r_bits     = initial_r_bits - QUOTIENT_BIT_STEP_SIZE;

    levels[next_level] = std::make_unique<growing_filter_type>(
        initial_q_bits, initial_r_bits, min_r_bits);
