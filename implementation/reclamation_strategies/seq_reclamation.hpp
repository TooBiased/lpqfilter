...
template <class T> class atomic_seq_reclamation
{
  private:
    T* ptr;
...
template <class T> class seq_reclamation_handle;


template <class T> class seq_reclamation_manager
{
  public:
  ...
  public:
    ...
    seq_reclamation_manager&
    operator=(const seq_reclamation_manager&) = default;
...
template <class T> class seq_reclamation_handle
{
  public:
    ...
    T* protect(atomic_seq_reclamation<T>& aptr) { return aptr.load(); }

    void unprotect_last(int = 1) { return; }

    void unprotect(T*) { return; }
...
{
  public:
    template <class T> using atomic_reclamation  = atomic_seq_reclamation<T>;
    template <class T> using reclamation_handle  = seq_reclamation_handle<T>;
    template <class T> using reclamation_manager = seq_reclamation_manager<T>;
};
