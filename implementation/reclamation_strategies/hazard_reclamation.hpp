...
template <class T>
using atomic_hazard = std::atomic<T*>;


enum class hazard_op_result {DELETED, NOT_DELETED};
...
public:
	...
	{
		for (size_t i = 0; i < MAX_THREADS; ++i) handles[i].store(other.handles[i]);
	}
...
private:
	...
	inline T* pop(T* ptr)
	{
		...
		if (pos < 0)
		{
			std::cout << "unsuccessful pop in hazard pointer not protected!" << std::endl;
		...
		if (nlastptr != lastptr)
		{
			...
			//std::cout << "edge case last is marked during pop in hazard_specific" << std::flush;
		}
		return temp;
	}
...
public:
    template <class T>
    using atomic_reclamation  = atomic_hazard<T>;
    template <class T>
    using reclamation_manager = hazard_manager<T>;
    template <class T>
    using reclamation_handle  = hazard_handle<T>;
