...
template <class Key, class Hash> class standard_qfilter_seq;
...
template <class Key, class Hash> class standard_qfilter_locking;
...
standard_qfilter_seq<K, H>::standard_qfilter_seq(size_t capacity,
    ...
    : cc(remainder_bits),
      ...
      fingerprint_mask((ONE << (cc.remainder_bits + quotient_bits)) - 1), hf(hf)
{
    table = std::make_unique<cell_type[]>(table_capacity);
}

template <class K, class H>
standard_qfilter_seq<K, H>::standard_qfilter_seq(
    const standard_qfilter_seq& other)
...
template <class K, class H>
standard_qfilter_seq<K, H>* standard_qfilter_seq<K, H>::create_bigger_QF(
...
// *****************************************************************************
template <class K, class H>
void standard_qfilter_seq<K, H>::prefetch(const hashed_type& hashed) const
{
...
template <class K, class H>
qf::InsertResult standard_qfilter_seq<K, H>::insert(const key_type& key)
{
...
standard_qfilter_seq<K, H>::insert_hash(const hashed_type& hashed)
{
    ...
    if (run.first == table_capacity - 1) { return qf::FAILED_OPERATION; }
    ...
    if (run_already_existing)
    {
        ...
        do {
            ...
            else if (table[insert_pos.first].remainder(cc, insert_pos.second) >
                     r)
                break;

            increment(insert_pos);
        } while (
            table[insert_pos.first].is_continuation(cc, insert_pos.second));

        // element is not present so we have to shift
        if (!has_space_for_shift(insert_pos)) { return qf::FAILED_OPERATION; }
...
template <class K, class H>
bool standard_qfilter_seq<K, H>::quick_insert(const key_type& key)
{
    return quick_insert_hash(hf(key));
}

template <class K, class H>
bool standard_qfilter_seq<K, H>::quick_insert_hash(const hashed_type& hashed)
{
    const auto [q, r] = this->get_quotient_and_remainder(hashed);
    const auto q_pos  = quotient_position(q);
...
// *** STATS *******************************************************************
// *****************************************************************************
template <class K, class H> size_t standard_qfilter_seq<K, H>::capacity() const
{
    return ONE << quotient_bits;
}

template <class K, class H>
size_t standard_qfilter_seq<K, H>::memory_usage_bytes() const
{
    return table_capacity * sizeof(cell_type);
}

template <class K, class H>
size_t standard_qfilter_seq<K, H>::unused_memory_bits() const
{
...
template <class K, class H>
double standard_qfilter_seq<K, H>::fill_level() const
{
    size_t used_slots = 0;
...
template <class K, class H>
void standard_qfilter_seq<K, H>::grow(base_qf& target_base)
{
...
template <class K, class H>
void standard_qfilter_seq<K, H>::increment(entry_pos& pos) const
{
    pos.second++;
...
template <class K, class H>
void standard_qfilter_seq<K, H>::decrement(entry_pos& pos) const
{
    if (pos.second == 0)
    {
...
template <class K, class H>
size_t standard_qfilter_seq<K, H>::offset(const entry_pos& pos) const
{
    return pos.first * cc.capacity + pos.second;
}

template <class K, class H>
size_t standard_qfilter_seq<K, H>::distance(const entry_pos& start,
...
{
    while (table[pos.first].is_shifted(cc, pos.second)) { decrement(pos); }
...
// assumes that pos is not empty
template <class K, class H>
bool standard_qfilter_seq<K, H>::has_space_for_shift(entry_pos pos) const
{
    while (!table[pos.first].is_empty(cc, pos.second)) { increment(pos); }
...
template <class K, class H>
void standard_qfilter_seq<K, H>::insert_entry_not_full(entry_type entry,
...
// which is lost due to the shift is written back to the entry reference
template <class K, class H>
void standard_qfilter_seq<K, H>::insert_entry_full(entry_type& entry,
                                                   entry_pos   insert_pos)
{
    const cell_type cell = table[insert_pos.first];
    cell_type       shifted_cell;
...
template <class K, class H>
void standard_qfilter_seq<K, H>::insert_and_shift(entry_type entry,
                                                  entry_pos& insert_pos)
{
...
// *** PRIVATE HELPER FUNCTIONS ************************************************
// *****************************************************************************
template <class K, class H> void standard_qfilter_seq<K, H>::print()
{
...
template <class K, class H>
void standard_qfilter_seq<K, H>::print_entry(entry_pos pos)
{
    print_entry(table[pos.first].entry(cc, pos.second), offset(pos));
}

template <class K, class H>
void standard_qfilter_seq<K, H>::print_entry(entry_type entry, size_t index)
{
...
template <class K, class H>
void standard_qfilter_seq<K, H>::print_entry(entry_type entry)
{
    std::cout << cell_type::entry_is_occupied(cc, entry) << "|"
              << cell_type::entry_is_continuation(cc, entry) << "|"
              << cell_type::entry_is_shifted(cc, entry) << " || "
...
template <class K, class H> bool standard_qfilter_seq<K, H>::check_consistency()
{
