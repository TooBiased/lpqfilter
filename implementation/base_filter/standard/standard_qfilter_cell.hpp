...
// members
	...
	static constexpr cell_base_type  read_lock_status     = occupied_mask
                                                            | continuation_mask;
...
public:
	...
	bool is_read_locked(const cstuff& cc, size_t start_pos,size_t end_pos) const
	{
		for (size_t pos = start_pos; pos < end_pos; pos++)
		{
			if (status(cc, pos) == read_lock_status)
				return true;
		}
		return false;
	}
    ...
    // setter with cstuff
    static void entry_set_remainder(const cstuff& cc,
                                    entry_type& entry,
	...
	static void entry_set_status(const cstuff&,
                                 entry_type& entry,
                                 status_type status)
	{
		entry &= ~status_mask;
	...
	static void entry_set_continuation(const cstuff&,
                                       entry_type& entry,
                                       bool val)
	{
	...
	static void status_set_occupied(const cstuff&,
                                    status_type& status,
                                    bool val)
	{
	...
	static void status_set_continuation(const cstuff&,
                                        status_type& status,
                                        bool val)
	{
...
struct standard_cell_non_atomic : public standard_cell_base<cell_type>
{
	...
	standard_cell_non_atomic& operator=(const standard_cell_non_atomic&)=default;
	...
	{
		const cell_type mask = base::shifted_mask <<base::shift_amount(cc, pos);
		this->cell = (this->cell & ~mask) | (mask * static_cast<int>(val));
	}

	void set_occupied(const cstuff& cc, bool val, size_t pos)
	{
		const cell_type mask = base::occupied_mask <<base::shift_amount(cc,pos);
		this->cell = (this->cell & ~mask) | (mask * static_cast<int>(val));
	}
    ...
    : standard_cell_base<std::atomic<cell_type>, cell_type>
{
	...
	void set_entry(const cstuff& cc, entry_type entry, size_t pos)
	{
		update_cell(static_cast<cell_type>(entry) <<base::shift_amount(cc, pos),
...
template<class cell_type, lock_tag tag>
class standard_cell_lock_guard
{
public:
	...
	standard_cell_lock_guard(const cstuff& cc,
                             entry_pos pos,
	...
	standard_cell_lock_guard& operator=(const standard_cell_lock_guard&)=delete;
...
template<class cell_type>
using compact_read_lock_guard  = standard_cell_lock_guard<cell_type,
                                                      lock_tag::read_lock_tag>;

template<class cell_type>
using compact_write_lock_guard = standard_cell_lock_guard<cell_type,
                                                      lock_tag::write_lock_tag>;
