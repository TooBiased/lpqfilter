...
protected:
	...
	standard_qfilter_conc_base& operator=(
        const standard_qfilter_conc_base& other)
	{
		table_status = other.table_status.load();
		return *this;
	}
	...
	standard_qfilter_conc_base& operator=(standard_qfilter_conc_base&&)=default;

	standard_qfilter_conc_base(qf::TableStatus table_status
...
public:
	...
	virtual void grow(standard_qfilter_conc_base<Key, Hash>& target_base,size_t block_size) = 0;
...
class standard_qfilter_conc
    : public standard_qfilter_conc_base<Key, Hash>
{
public:
...
private:
	...
	void increment_and_update(entry_pos& pos,
                              cell_type& cell, entry_type& entry) const;
	...
	entry_pos unsafe_find_run_start(entry_pos cluster_start,
                                    entry_pos pos) const;
...
(size_t capacity, size_t remainder_bits, const hash_function_type& _hf)
	 : base_qf(),
       ...
       fingerprint_mask((ONE << static_cast<size_t>(
                             remainder_bits + this->quotient_bits)) - 1),
       hf(_hf)
{
...
template<class K, class H>
standard_qfilter_conc<K,H>*
standard_qfilter_conc<K,H>::create_bigger_QF(
...
template<class K, class H>
qf::InsertResult
standard_qfilter_conc<K,H>::insert(const key_type& key)
{
...
standard_qfilter_conc<K,H>::insert_hash(const hashed_type& hashed)
{
    ...
    if (cc.capacity > 1)
    {
        ...
        if (lock_elision_result.successful())
    ...
    const bool run_already_existing = table[q_pos.first].is_occupied(cc,
                                                                  q_pos.second);
    ...
    if (run_already_existing)
    {
        ...
        if (insert_pos == run)
        {
            ...
            cell_type::entry_set_remainder(cc, new_entry,
                                           cell_type::entry_remainder(cc,
                                                                current_entry));
...
template<class K, class H>
bool
standard_qfilter_conc<K,H>::quick_insert(const key_type& key)
{
...
template<class K, class H>
bool
standard_qfilter_conc<K,H>::quick_insert_hash(const hashed_type& hashed)
{
    const auto[q, r] = this->get_quotient_and_remainder(hashed);
    const entry_pos q_pos = quotient_position(q);
...
template<class K, class H>
qf::ContainsResult
standard_qfilter_conc<K,H>::contains(const key_type& key)
{
...
standard_qfilter_conc<K,H>::contains_hash(const hashed_type& hashed)
{
    ...
    read_lock_guard read_lock(cc);
    auto iter = find_run_start(find_and_lock_cluster_start(q_pos, read_lock),
                               q_pos);
...
template<class K, class H>
qf::ContainsResult
standard_qfilter_conc<K,H>::unsafe_contains_hash(
    const hashed_type& hashed) const
{
    const auto[q, r] = this->get_quotient_and_remainder(hashed);
    const entry_pos q_pos = quotient_position(q);
...
standard_qfilter_conc<K,H>::get_quotient_and_remainder(
    const hashed_type&hashed) const
{
    return qf::get_quotient_and_remainder<entry_type>(hashed,
                                                      this->quotient_bits,
                                                      cc.remainder_bits);
}
...
template<class K, class H>
size_t
standard_qfilter_conc<K,H>::capacity() const
{
...
template<class K, class H>
void
standard_qfilter_conc<K,H>::grow(base_qf& target_base, size_t block_size)
{
    if (cc.remainder_bits <= 1)
    {
        return;
...
template<class K, class H>
void
standard_qfilter_conc<K,H>::increment(entry_pos& pos) const
{
    pos.second++;
...
template<class K, class H>
void
standard_qfilter_conc<K,H>::decrement(entry_pos& pos) const
{
    if (pos.second == 0)
    {
...
template<class K, class H>
size_t
standard_qfilter_conc<K,H>::offset(const entry_pos& pos) const
{
...
template<class K, class H>
size_t
standard_qfilter_conc<K,H>::distance(const entry_pos& start,
...
template<class K, class H>
ContainsResultEnum
standard_qfilter_conc<K,H>::try_contains_lock_elision(const cell_type cell,
                                               const entry_pos pos,
                                               const entry_type remainder) const
{
...
template<class K, class H>
qf::InsertResult
standard_qfilter_conc<K,H>::try_insert_lock_elision(cell_type cell,
                                               const entry_pos pos,
                                               const entry_type remainder) const
{
    auto original_cell = cell;
    if (cell.is_locked(cc, pos.second))
        return qf::FAILED_OPERATION;
    ...
    if (!run_already_existing)
        cell.set_occupied(cc, true, pos.second);
...
template<class K, class H>
void
standard_qfilter_conc<K,H>::increment_and_update(entry_pos& pos,
                                                 cell_type& cell,
...
template<class K, class H>
void
standard_qfilter_conc<K,H>::decrement_and_update(entry_pos& pos,
                                                 cell_type& cell,
                                                 entry_type& entry) const
{
    if (pos.second == 0)
...
template<class K, class H>
void
standard_qfilter_conc<K,H>::force_update(const entry_pos& pos,
                                         cell_type& cell,
                                         entry_type& entry) const
{
    cell = table[pos.first];
...
template<class K, class H>
bool
standard_qfilter_conc<K,H>::try_empty_slot(cell_type local_cell,
                                           entry_pos pos,
                                           entry_type remainder)
{
    ...
    while (true)
    {
        ...
        if (table[pos.first].CAS(local_cell, filled_cell))
            return true;
...
template<class K, class H>
bool
standard_qfilter_conc<K,H>::unsafe_try_empty_slot(entry_pos pos,
                                                  entry_type remainder)
{
...
template<class K, class H>
bool
standard_qfilter_conc<K,H>::has_space_and_lock(entry_pos pos,
                                               write_lock_guard& write_lock)
{
    increment(pos);
...
template<class K, class H>
void
standard_qfilter_conc<K,H>::get_growing_write_lock(entry_pos pos,
                                                   write_lock_guard& write_lock)
{
    ...
    while (true)
    {
        ...
        if (local_cell.is_empty(cc, pos.second))
        {
            ...
            if (table[pos.first].CAS(local_cell, locked_cell))
            {
                write_lock.set_data(&table[pos.first], pos, cell_type::write_lock_status);
                return;
            }
        }
        else
...
template<class K, class H>
typename standard_qfilter_conc<K,H>::entry_pos
standard_qfilter_conc<K,H>::find_and_lock_cluster_start(entry_pos pos,
                                                        read_lock_guard& read_lock)
{
    cell_type local_cell = table[pos.first];
    entry_type local_entry = local_cell.entry(cc, pos.second);
                                           ...
                                           bool set_occupied) const
{
    ...
    if (set_occupied)
        table[pos.first].set_occupied(cc, true, pos.second);
...
template<class K, class H>
bool
standard_qfilter_conc<K,H>::acquire_read_lock(entry_pos pos,
                                              read_lock_guard& read_lock)
{
    ...
    while (true)
    {
        ...
        if (!local_cell.is_cluster_start(cc, pos.second))
        {
            return false;
        }
...
template<class K, class H>
void
standard_qfilter_conc<K,H>::insert_and_shift(entry_type entry,
                                             entry_pos& insert_pos,
                                             write_lock_guard& write_lock,
                                             read_lock_guard& read_lock)
{
    if (insert_pos == read_lock.pos)
    {
        ...
        const entry_type remainder = table[insert_pos.first].remainder(cc,
                                                             insert_pos.second);
        table[insert_pos.first].set_remainder(cc,
                                          cell_type::entry_remainder(cc, entry),
                                          insert_pos.second);

        const auto status = read_lock.status;
        read_lock.status = cell_type::entry_status(cc, entry);
        cell_type::status_set_occupied(cc, read_lock.status,
                                       cell_type::entry_is_occupied(cc,
                                                                    status));
...
template<class K, class H>
void
standard_qfilter_conc<K,H>::insert_entry_not_full(entry_type entry,
                                                  entry_pos insert_pos,
                                                  size_t write_lock_pos)
{
    cell_type cell;
    ...
    do
    {
        ...
        } while (cell.is_read_locked(cc,
                                     read_lock_check_start,
                                     write_lock_pos));
...
template<class K, class H>
void
standard_qfilter_conc<K,H>::insert_entry_full(entry_type& entry,
                                                           entry_pos insert_pos)
{
    cell_type cell;
    cell_type shifted_cell;
...
template<class K, class H>
void
standard_qfilter_conc<K,H>::grow(next_bigger_qf& target,
                                              size_t block_start,
                                              size_t block_size)
{
    read_lock_guard read_lock(cc);
    ...
    const entry_pos block_end{std::min(block_start +block_size, table_capacity),
                              0 };
    entry_type local_entry = table[cluster_start.first].entry(cc,
                                                          cluster_start.second);
    ...
    if (cell_type::entry_is_write_locked(cc, local_entry))
    {
        ...
        local_entry = table[cluster_start.first].entry(cc,
                                                       cluster_start.second);
    }
...
template<class K, class H>
typename standard_qfilter_conc<K,H>::entry_pos
standard_qfilter_conc<K,H>::grow_cluster(next_bigger_qf& target,
                                         const read_lock_guard& read_lock) const
{
    ...
    while (!cell_type::entry_is_empty(cc, source_entry))
    {
        ...
        else
        {
            ...
            target.table[target_pos.first].set_occupied(target.cc,
                                                        true,
                                                        target_pos.second);
        }
...
template<class K, class H>
void
standard_qfilter_conc<K,H>::print() const
{
...
template<class K, class H>
void
standard_qfilter_conc<K,H>::print(entry_pos start,
                                               entry_pos end) const
{
...
template<class K, class H>
void
standard_qfilter_conc<K,H>::print(size_t start,
                                               size_t count) const
{
...
template<class K, class H>
void
standard_qfilter_conc<K,H>::print_cell(size_t table_pos) const
{
...
template<class K, class H>
void
standard_qfilter_conc<K,H>::print_cell(cell_type cell,
...
template<class K, class H>
void
standard_qfilter_conc<K,H>::print_entry(entry_pos pos) const
{
...
template<class K, class H>
void
standard_qfilter_conc<K,H>::print_entry(entry_type entry,
                                                     entry_pos pos) const
{
...
template<class K, class H>
void
standard_qfilter_conc<K,H>::print_entry(entry_type entry) const
{
    std::cout << cell_type::entry_is_occupied(cc, entry)     << "|"
              << cell_type::entry_is_continuation(cc, entry) << "|"
              << cell_type::entry_is_shifted(cc, entry)      << " ||     "
...
template<class K, class H>
bool
standard_qfilter_conc<K,H>::check_consistency() const
{
