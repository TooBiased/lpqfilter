...
public:
	...
	inline std::pair<size_t, size_t> get_quotient_and_remainder(const key_type& key) const
	{
		auto[q, r] = qf::get_quotient_and_remainder<size_t>(hf(key), quotient_bits, cc.remainder_bits);
	...
	{
		return table_capacity * (sizeof(cell_type) * 8 - cc.capacity * cc.remainder_bits);
	}
