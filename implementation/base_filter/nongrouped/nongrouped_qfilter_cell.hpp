#pragma once
/*******************************************************************************
 * implementation/base_filter/nongrouped/nongrouped_qfilter_cell.hpp
 *
 * slot class for the non-grouped quotient filter variants
 *
...
struct nongrouped_cell_base
{
    ...
    bool is_occupied() const { return entry & occupied_mask; }

    bool is_continuation() const { return entry & continuation_mask; }

    bool is_shifted() const { return entry & shifted_mask; }

    bool is_empty() const { return status() == 0; }

    bool is_cluster_start() const { return status() == cluster_start_status; }

    bool is_write_locked() const { return status() == write_lock_status; }

    bool is_read_locked() const { return status() == read_lock_status; }

    bool is_locked() const { return is_locked(status()); }
    ...
    basic_entry_type remainder() const { return entry >> status_bits; }
    ...
    static constexpr basic_entry_type read_lock_status =
        occupied_mask | continuation_mask;
};

template <class entry_type> struct nongrouped_cell_atomic;
...
struct nongrouped_cell_non_atomic : nongrouped_cell_base<entry_type, entry_type>
{
    ...
    nongrouped_cell_non_atomic(const atomic& other) : base(other.entry.load())
    {
    }

    nongrouped_cell_non_atomic&
    operator=(const nongrouped_cell_non_atomic&) = default;
    ...
    {
        this->entry =
            (this->entry & base::status_mask) | (remainder << status_bits);
    }
    ...
    void set_write_lock() { set_status(base::write_lock_status); }

    void set_read_lock() { set_status(base::read_lock_status); }

    void or_status(status_type status) { this->entry |= status; }
};
    ...
    : nongrouped_cell_base<std::atomic<entry_type>, entry_type>
{
    ...
    operator non_atomic() { return {this->entry.load()}; }
...
template <class entry_type> struct nongrouped_cell_lock_guard
{
    ...
    nongrouped_cell_lock_guard&
    operator=(const nongrouped_cell_lock_guard&) = delete;

    nongrouped_cell_lock_guard(nongrouped_cell_lock_guard&& other)
        : pos(std::move(other.pos)), slot(std::move(other.slot)),
    ...
    void set_data(size_t pos, nongrouped_cell_atomic<entry_type>& locked_slot,
                  status_type status)
    {
        if (this->pos != pos)
        {
