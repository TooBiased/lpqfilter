#pragma once
/*******************************************************************************
 * implementation/base_filter/nongrouped/nongrouped_qfilter_conc.hpp
 *
 * concurrent quotient filter with non-grouped slots (one slot per atomic)
 *
...
class nongrouped_qfilter_conc
{
  ...
  private:
    ...
    bool try_empty_slot(size_t pos, entry_type remainder)
    {
        ...
        while (true)
        {
            for (local_entry = table[pos]; local_entry.is_write_locked();
            ...
            if (!local_entry.is_empty()) return false;

            if (table[pos].CAS(local_entry, filled_entry)) return true;
        }
    }
    ...
    size_t find_cluster_start(size_t pos) const
    {
        ...
        while (!local_entry.is_cluster_start()) { local_entry = table[--pos]; }

        return pos;
    }

    size_t find_run_start(size_t cluster_start, size_t pos,
                          read_lock_guard& read_lock, bool set_occupied = false)
    {
        size_t bucket = cluster_start;
        size_t run    = bucket;

        if (set_occupied) set_occupied_atomic(pos, read_lock);
    ...
    void
    insert_and_shift(cell entry, size_t& insert_pos,
                     write_lock_guard& write_lock, read_lock_guard& read_lock)
    {
        if (insert_pos == read_lock.pos)
        {
            ...
            cell::status_set_occupied(
                read_lock.status,
                cell::status_is_occupied(old_read_lock_status));
        ...
        while (insert_pos != write_lock.pos)
        {
            do {
                ...
                if (expected.is_occupied()) { current.set_occupied(true); }
    ...
    void set_occupied_atomic(size_t pos)
    {
        ...
        while (true)
        {
            ...
            if (table[pos].CAS(local_entry, new_entry)) break;
        }
    }
    ...
    bool acquire_read_lock(size_t pos, read_lock_guard& read_lock)
    {
        ...
        while (true)
        {
            ...
            if (!local_entry.is_cluster_start()) { return false; }
    ...
    void
    grow(nongrouped_qfilter_conc& target, size_t block_start, size_t block_size)
    {
        read_lock_guard  read_lock;
        write_lock_guard write_lock;
        size_t           cluster_start = block_start;
        const size_t     block_end =
            std::min(block_start + block_size, table_capacity);
    ...
    size_t
    grow_cluster(nongrouped_qfilter_conc& target, read_lock_guard& read_lock)
    {
        ...
        while (!source_entry.is_empty())
        {
            ...
            {
                target.set_occupied_atomic(
                    run_start * 2 + static_cast<size_t>(last_leading_bit));
            }
  ...
  public:
          ...
          quotient_bits(qf::capacity_to_quotient_bits(capacity))
    {
        ...
        fingerprint_mask =
            (ONE << static_cast<size_t>(remainder_bits + quotient_bits)) - 1;
    }
    ...
    nongrouped_qfilter_conc(const nongrouped_qfilter_conc& other)
        : table_capacity(other.table_capacity),
          ...
          table_status(other.table_status), hf(other.hf),
    ...
    bool insert(const Key& key) { return insert_hash(hf(key)); }
    ...
    qf::InsertResult insert_hash(const HashVal& hashed)
    {
        ...
        if (try_empty_slot(q, r)) return 1;

        write_lock_guard write_lock;
        if (!has_space_and_lock(q, write_lock)) return qf::FAILED_OPERATION;
        ...
        size_t       run =
            find_run_start(cluster_start, q, read_lock, !run_already_existing);
    ...
    bool quick_insert(const Key& key) { return quick_insert_hash(hf(key)); }
    ...
    bool quick_insert_hash(const HashVal& hashed)
    {
        ...
        if (unsafe_try_empty_slot(q, r)) return 1;
    ...
    void print() { print(0, table_capacity); }
    ...
    {
        if (table_status != qf::TableStatus::growing) return;
