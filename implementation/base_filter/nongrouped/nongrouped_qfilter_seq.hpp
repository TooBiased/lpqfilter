...
public:
	...
	size_t max_super_cluster_length()
	{
		return max_super_cluster;
	}

	size_t get_run_count(size_t length)
	{
		return get_count(runs, length);
	}
	...
	void add_run(size_t& val)
	{
		add_entry(runs, val);
	}

	void add_cluster(size_t& val)
	{
		add_entry(cluster, val);
	}
...
private:
	...
	static size_t grow_cluster(nongrouped_qfilter_seq& source,
                               nongrouped_qfilter_seq& target,
                               size_t cluster_start)
	{
		...
		while (!source_entry.is_empty())
		{
			...
			{
				target.table[run_start*2 +static_cast<size_t>(last_leading_bit)]
                    .set_occupied(true);
			}
...
public:
	...
	{
		return table_capacity * (sizeof(cell) * 8
                                 - remainder_bits - qf::status_bits);
	}
	...
	void grow(nongrouped_qfilter_seq& target)
	{
		...
		while (table[cluster_start].is_empty())
		{
			cluster_start++;
		}
