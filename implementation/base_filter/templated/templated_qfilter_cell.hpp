...
template <size_t entry_bits> struct entry_type_chooser
{
    using type = typename entry_type_chooser<entry_bits - 1>::type;
};

template <> struct entry_type_chooser<0>
{
    using type = std::uint8_t;
};

template <> struct entry_type_chooser<9>
{
    using type = std::uint16_t;
};

template <> struct entry_type_chooser<17>
{
    using type = std::uint32_t;
};

template <> struct entry_type_chooser<33>
{
    using type = std::uint64_t;
};

template <size_t remainder_bits>
using min_entry_type =
    typename entry_type_chooser<remainder_bits + status_bits>::type;
template <size_t remainder_bits>
...
struct templated_cell_base
{
  public:
    ...
    static constexpr cell_base_type generate_full_occupied_mask()
    {
        ...
        {
            mask <<= remainder_bits; // two shifts to prevent shift overflow /
                                     // undefined behavior
            mask <<= status_bits;
            mask |= occupied_mask;
        }
  ...
  public:
    ...
    bool is_occupied(size_t pos) const { return status(pos) & occupied_mask; }
    ...
    bool is_shifted(size_t pos) const { return status(pos) & shifted_mask; }
    ...
    bool is_empty(size_t pos) const { return status(pos) == 0; }
    ...
    bool is_read_locked(size_t start_pos, size_t end_pos) const
    {
        ...
        {
            if (status(pos) == read_lock_status) return true;
        }
    ...
    static constexpr size_t capacity =
        sizeof(cell_base_type) * 8u / (remainder_bits + status_bits);
    static constexpr cell_base_type one = static_cast<cell_base_type>(1u);

    static constexpr cell_base_type
        entry_mask = (~static_cast<cell_base_type>(0)) >>
                     (sizeof(cell_base_type) * 8u -
                      (status_bits + remainder_bits));
    static constexpr cell_base_type remainder_mask =
        (one << remainder_bits) - 1;
    ...
    static constexpr cell_base_type full_occupied_mask =
        generate_full_occupied_mask();
    static constexpr cell_base_type full_shifted_mask =
        generate_full_shifted_mask();
...
template <class cell_type, size_t remainder_bits> struct templated_cell_atomic;
    ...
    : public templated_cell_base<cell_type, remainder_bits>
{
    ...
    templated_cell_non_atomic&
    operator=(const templated_cell_non_atomic&) = default;
    ...
    {
        this->cell &=
            ~(base::remainder_mask << (base::shift_amount(pos) + status_bits));
    ...
    : templated_cell_base<std::atomic<cell_type>, remainder_bits, cell_type>
{
    ...
    templated_cell_atomic(const non_atomic& other) : base(other.cell) {}
    ...
    void set_read_lock(size_t pos) { set_status(base::read_lock_status, pos); }
...
template <class cell_type> struct templated_cell_lock_guard
{
    using entry_pos = entry_position<cell_type::capacity>;

    templated_cell_lock_guard() = default;
    templated_cell_lock_guard(entry_pos pos, cell_type& locked_cell,
                              status_type status)
    ...
    templated_cell_lock_guard&
    operator=(const templated_cell_lock_guard&) = delete;

    templated_cell_lock_guard(templated_cell_lock_guard&& other)
        : pos(std::move(other.pos)), cell(std::move(other.cell)),
