...
class templated_qfilter_conc_base
{
  ...
  protected:
    ...
    templated_qfilter_conc_base&
    operator=(const templated_qfilter_conc_base& other)
    {
    ...
    templated_qfilter_conc_base&
    operator=(templated_qfilter_conc_base&& other) = default;
    templated_qfilter_conc_base(
        size_t          capacity     = qf::DEFAULT_MIN_CAPACITY,
  ...
  public:
    ...
    using instanciated =
        templated_qfilter_conc<key_type, rem_bits, hash_function_type>;
...
class templated_qfilter_conc : public templated_qfilter_conc_base<Key, Hash>
{
  public:
    ...
    using next_smaller_qf =
        templated_qfilter_conc<Key, remainder_bits + 1, Hash>;
    using next_bigger_qf =
        templated_qfilter_conc<Key, remainder_bits - 1, Hash>;
  ...
  private:
                            ...
                            const entry_type remainder) const
    {
        ...
        if (cell.is_locked(pos.cell)) return qf::FAILED_OPERATION;
        ...
        if (!run_already_existing) cell.set_occupied(true, pos.cell);
        ...
        if (run_already_existing)
        {
            do {
                if (cell.remainder(insert_pos) == remainder)
                    return insert_pos - cluster_start; // element already
                                                       // present

                if (cell.remainder(insert_pos) > remainder) break;
    ...
    {
        ++pos;
        if (pos.cell == 0) { cell = table[pos.table]; }
    ...
    {
        --pos;
        if (pos.cell == cell_type::capacity - 1) { cell = table[pos.table]; }
        entry = cell.entry(pos.cell);
    }

    void
    force_update(const entry_pos& pos, cell_type& cell, entry_type& entry) const
    {
    ...
    bool has_space_and_lock(entry_pos pos, write_lock_guard& write_lock)
    {
        ...
        while (pos.table != table_capacity - 1)
        {
            ...
            if (local_cell.is_empty(pos.cell))
            {
                ...
                {
                    write_lock.set_data(&table[pos.table], pos,
    ...
    find_and_lock_cluster_start(entry_pos pos, read_lock_guard& read_lock)
    {
        ...
        while (true)
        {
            ...
            if (cell_type::entry_is_cluster_start(local_entry))
            {
                ...
                if (table[pos.table].CAS(local_cell, locked_cell))
                {
                    read_lock.set_data(&table[pos.table], pos,
                                       local_cell.status(pos.cell));
                    return pos;
                }
                             ...
                             bool set_occupied = false) const
    {
        ...
        if (set_occupied) table[pos.table].set_occupied(true, pos.cell);
    ...
    bool acquire_read_lock(entry_pos pos, read_lock_guard& read_lock)
    {
        ...
        while (true)
        {
            ...
            if (!local_cell.is_cluster_start(pos.cell)) { return false; }
                     ...
                     write_lock_guard& write_lock, read_lock_guard& read_lock)
    {
        if (insert_pos == read_lock.pos)
        {
            ...
            const entry_type remainder =
                table[insert_pos.table].remainder(insert_pos.cell);
            table[insert_pos.table].set_remainder(
                cell_type::entry_remainder(entry), insert_pos.cell);

            const auto status = read_lock.status;
            read_lock.status  = cell_type::entry_status(entry);
            cell_type::status_set_occupied(
                read_lock.status, cell_type::entry_is_occupied(status));
                               ...
                               size_t write_lock_pos)
    {
        ...
        do {
            ...
            do {
                ...
                cell.is_read_locked(read_lock_check_start, write_lock_pos));
    ...
    void grow(next_bigger_qf& target, size_t block_start, size_t block_size)
    {
        ...
        entry_type local_entry =
            table[cluster_start.table].entry(cluster_start.cell);
    ...
    entry_pos
    grow_cluster(next_bigger_qf& target, const read_lock_guard& read_lock) const
    {
  ...
  public:
    ...
    templated_qfilter_conc(const templated_qfilter_conc& other)
        : base_qf(other), table_capacity(other.table_capacity),
    ...
    qf::InsertResult insert_hash(const hashed_type& hashed) override
    {
        ...
        if constexpr (cell_type::capacity > 1)
        {
            ...
            if (lock_elision_res.successful()) return lock_elision_res;
        }
        ...
        const auto      cluster_start =
            find_and_lock_cluster_start(q_pos, read_lock);
        const auto run =
        ...
        if (run_already_existing)
        {
            ...
            if (insert_pos == run)
            {
                ...
                cell_type::entry_set_remainder(
                    new_entry, cell_type::entry_remainder(current_entry));
    ...
    qf::ContainsResult contains_hash(const hashed_type& hashed) override
    {
        ...
        if constexpr (cell_type::capacity > 1)
        {
            auto lock_elision_result =
                try_contains_lock_elision(q_cell, q_pos, r);
            if (lock_elision_result != ContainsResultEnum::FAILED_OPERATION)
                return lock_elision_result;
        }

        read_lock_guard read_lock;
        auto            iter = find_run_start(
            find_and_lock_cluster_start(q_pos, read_lock), q_pos);
    ...
    void print() const { print(0, table_capacity); }
    ...
    bool check_consistency() const override
    {
        ...
        for (entry_pos pos{0, 0}; pos.table < table_capacity; ++pos)
        {
            ...
            if (cell_type::entry_is_continuation(current) &&
