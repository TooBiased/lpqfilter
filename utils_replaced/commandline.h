...
public:
	...
	IntType intArg(const std::string& name, IntType def = 0)
    {
        ...
        if (ind+1 < params.size())
        {
            ...
            {  
				...
				{
					r = std::stoll(params[ind + 1]);
				}
				else if constexpr (std::is_same_v<IntType, unsigned int> || std::is_same_v<IntType, unsigned long>)
				{
					r = std::stoul(params[ind + 1]);
				}
				else if constexpr (std::is_same_v<IntType, unsigned long long>)
				{
					r = std::stoull(params[ind + 1]);
				}
			}
