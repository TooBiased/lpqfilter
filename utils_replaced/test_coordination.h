...
template<typename MFunctor, typename TFunctor, typename QuotientFilter, typename Config>
inline int start_threads(MFunctor mf, TFunctor tf, QuotientFilter& filter, Config config)
{
    std::thread* local_thread = new std::thread[config.p-1];

    for (size_t i = 0; i < config.p-1; ++i)
    {
        local_thread[i] = std::thread(std::forward<TFunctor>(tf), std::ref(filter), config, i+1);
    }

    int temp = std::forward<MFunctor>(mf)(filter, config, 0);
...
static const size_t parallel_exec_block_size = 4096;
...
{
    auto c_s = global_counter.fetch_add(parallel_exec_block_size);
    while (c_s < e)
    {
        auto c_e = std::min(c_s+parallel_exec_block_size, e);
        for (size_t i = c_s; i < c_e; ++i)
            f(i, std::forward<Types>(param)...);
        c_s = global_counter.fetch_add(parallel_exec_block_size);
    }
}
