...
struct test
{
    ...
    struct test_filter
    {
        static int execute(ThreadType t, FilterType& filter, fill_benchmark_config config)
        {
            ...
            for (size_t i = 0; i < config.it * config.sub_it; ++i)
            {
                ...
                for ( size_t fill_step = 1; fill_step < max_steps; ++fill_step )
                {
                    ...
                    // STAGE 2 - Contains for some randomly chosen inserted elements
                    {
                        ...
                        fill_loc_succ(loc_buffer, t.id, inserted, local_test_size);

                        auto dur = t.synchronized(contains_successful<FilterType>, filter, loc_buffer, local_test_size);
                    ...
                    // STAGE 3 - Contains for non-inserted elements
                    {
                        ...
                        auto dur = t.synchronized(contains_unsuccessful<FilterType>, filter, loc_buffer, local_test_size);
                    ...
                    // STAGE 4 - Insert Test
                    {
                        fill_loc_insert(loc_buffer, t.id, inserted, local_test_size);

                        auto dur = t.synchronized(insert_test<FilterType>, filter, loc_buffer, local_test_size);

                        t.out << otm::width(10) << dur.second/1000000.;
                        inserted += config.test_size;
                    }

                    t.out << otm::width(10) << double(false_positives.load()) / config.test_size * 100
...
int main(int argn, char** argc)
{
	...
	config.set_fp_rate_percent(c.double_arg("-fp_rate_%", config.fp_rate * 100));
