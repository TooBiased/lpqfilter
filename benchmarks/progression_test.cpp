...
void print_parameters(size_t it, size_t p, double fp_rate, size_t n, size_t cap,
                      size_t sample)
{
    out(it, 3);
    out(p, 3);
...
void fill_succ(size_t max_index, size_t count, size_t sample)
{
    ...
    for (size_t i = 0; i < count; i++) { keys_succ[i] = keys[dis(re)]; }
}

template <class QuotientFilter> int insert(QuotientFilter& filter, size_t end)
{
    const size_t off = offset;
    ttm::execute_parallel(current_block, end, [&filter, off](size_t i) {
        auto key = keys[off + i];
...
int contains_successful(QuotientFilter& filter, size_t end)
{
    const size_t off = offset;
    size_t       err = 0;
    ttm::execute_parallel(current_block, end, [&filter, &err, off](size_t i) {
...
int contains_unsuccessful(QuotientFilter& filter, size_t end)
{
    const size_t off = offset;
    size_t       fp  = 0;
    ttm::execute_parallel(current_block, end, [&filter, &fp, off](size_t i) {
...
template <class FilterType> struct test
{
    ...
    {
        static int execute(ThreadType t, FilterType& filter,
                           progression_benchmark_config config)
        {
            ...
            for (size_t i = 0; i < config.it * config.sub_it; ++i)
            {
                ...
                // STAGE 0.3 - Reinitialize the quotient filter + some
                // synchronization
                if (ThreadType::is_main)
                ...
                for (size_t sample = 0; sample < config.sample_count; sample++)
                {
                    if (ThreadType::is_main)
                    {
                        ...
                        print_parameters(i, config.p, config.fp_rate, config.n,
                                         start_cap, sample);
                    }
                    ...
                    // STAGE 1 - Insert
                    {
                        ...
                        auto dur = t.synchronized(insert<HandleType>, handle,
                                                  sample_size);
                    ...
                    // STAGE 2 - Successful Contains
                    {
                        if (ThreadType::is_main)
                        {
                            fill_succ(offset + sample_size, sample_size,
                                      sample);
                            current_block.store(0);
                        }
                        ...
                        auto dur =
                            t.synchronized(contains_successful<HandleType>,
                                           handle, sample_size);
                        t.out << otm::width(10) << dur.second / 1000000.;
                    }
                    ...
                    // STAGE 3 - Unsuccessful Contains
                    {
                        ...
                        auto dur =
                            t.synchronized(contains_unsuccessful<HandleType>,
                                           handle, sample_size);
                        t.out << otm::width(10) << dur.second / 1000000.
                              << otm::width(10)
                              << double(false_positives.load()) / sample_size *
                                     100
...
int main(int argn, char** argc)
{
    ...
    {
        std::cout
            << "Error: only one of \"-growing\" or \"-dynamic\" can be used\n";
        return 1;
    }
    ...
    if (config.growing)
    {
        ...
        {
            std::cout
                << "Error: this quotient filter is not growing_compatible\n";
            return 1;
        }
    ...
    else
    {
        using qfilter_type = typename instanciation_handler<
            QF_type<key_type>, QF_type<key_type>::is_templated>::type;
