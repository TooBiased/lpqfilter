...
{
    size_t err = 0;
    ttm::execute_parallel(current_block, end, [&filter, &err](size_t i) {
        const auto key = keys[i];
...
{
    size_t fp = 0;
    ttm::execute_parallel(current_block, end, [&filter, &fp](size_t i) {
...
{
    template <class ThreadType> struct filter_test
    {
        static int
        execute(ThreadType t, FilterType& filter, basic_benchmark_config config)
        {
            ...
            for (size_t i = 0; i < config.it * config.sub_it; ++i)
            {
                ...
                {
                    FilterType newlocal =
                        construct_quotient_filter<FilterType>(config);
                ...
                // STAGE 1 - Insert
                {
                    ...
                    auto dur =
                        t.synchronized(insert<HandleType>, handle, config.n);
                ...
                // STAGE 2 - Successful Contains
                {
                    ...
                    auto dur = t.synchronized(contains_successful<HandleType>,
                                              handle, config.n);
                    t.out << otm::width(10) << dur.second / 1000000.;
                }
                ...
                // STAGE 3 - Unsuccessful Contains
                {
                    ...
                    auto dur = t.synchronized(contains_unsuccessful<HandleType>,
                                              handle, config.n);
                    t.out << otm::width(10) << dur.second / 1000000.
                          << otm::width(10)
                          << double(false_positives.load()) / config.n * 100
